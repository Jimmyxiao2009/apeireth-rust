...
- Endpoints: 7/9 pass, 1 fail, 1 error
...
|--------|------|--------|---------|-------|----|------|
| GET | `/healthz` | 200 | PASS | ✓ | 1.59 | liveness probe |
| GET | `/status` | 200 | PASS | ✓ | 7.68 | module status cross-check |
| GET | `/pipeline/adversarial` | 200 | PASS | ✓ | 2631.01 | 30/30 adversarial suite |
| POST | `/pipeline/lint?policy=STANDARD` | 200 | FAIL | ✗ | 19.67 | safe spec under STANDARD policy |
  - error: `missing keys: ['spec', 'counts']`
| POST | `/pipeline/policy-gate?policy=PERMISSIVE` | 200 | PASS | ✓ | 0.98 | bad spec under PERMISSIVE policy |
| POST | `/pipeline/run` | 200 | PASS | ✓ | 48.25 | mixed JSONL run: 1 safe + 1 bad |
| GET | `/no-such-endpoint` | 404 | PASS | ✓ | 0.82 | sad: unknown path returns 404 |
| POST | `/healthz` | 405 | PASS | ✓ | 17.64 | sad: POST on GET-only endpoint returns 405 |
| POST | `/pipeline/lint` | — | ERROR | ✗ | 0.61 | sad: oversize body returns 413 |
  - error: `http_call: [WinError 10053] 你的主机中的软件中止了一个已建立的连接。`
