...
pub fn chat_streaming(input: &str, history: &[ChatMessage], sender: &std::sync::mpsc::Sender<String>) -> String {
    // 跟 chat() 同样路径, 但走 stream
    let store = match memory_store() {
        Ok(s) => s,
        Err(e) => {
            // memory store 初始化失败 → fallback 走 chat() (no streaming)
    ...
    chat_internal_streaming(input, history, &store, sender)
}
    ...
    match call_llm_stream_sync(input, history, sender) {
        Ok(reply) => {
            ...
            R19_TOKEN_USED.fetch_add(
                r19_token_compute(input) + r19_token_compute(&reply.text),
                Ordering::Relaxed,
            );
            reply.text
        }
...
pub fn chat(input: &str, history: &[ChatMessage]) -> String {
    ...
    chat_internal(input, history, &store, call_llm_sync)
}
...
mod system_prompt_tests {
    ...
    fn prompt_is_concise() {
        // 当前阶段基地未成熟, prompt 应保持极简
        // 主人: "简洁直接, 工程风格" — 估算 < 100 字符
        assert!(
            SYSTEM_PROMPT.len() < 200,
            "当前 prompt 应 < 200 字符 (极简, 等基地真成熟再扩)"
        );
    }
}
...
mod organs_real_backend_tests {
    ...
    fn perception_health_uses_real_distinct_session_count() {
        ...
        // 没有 episode → 0 session → health = 0.0
        assert_eq!(organ.health, 0.0, "无 episode → 0 session → 0/5 = 0.0");
    }
