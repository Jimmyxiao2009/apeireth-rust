...
use apeireth_protocol::{
    is_tool_result_error, MessageRole, NormalizedFinishReason, NormalizedMessage, NormalizedRequest,
    NormalizedResponse, NormalizedTool, NormalizedToolChoice, ProtocolKind, ProtocolRouter, ToolCall,
};
use serde_json::json;
...
fn make_fake_response(kind: ProtocolKind) -> serde_json::Value {
    // 模拟协议特定的 raw JSON 响应 (fake, 不调 LLM)
    match kind {
...
fn main() {
    println!("apeireth-protocol router_demo (4 协议归一化, fake JSON, 不调 LLM)");
    println!("============================================================");
    ...
    // ============================================================
    print_section("1. NormalizedRequest → 4 协议 JSON (encode)");

    for kind in ProtocolRouter::supported_protocols() {
        let adapter = router.adapter(*kind);
        match adapter.adapt_request(&req) {
            Ok(v) => {
                println!("\n[{}] {} (POST {})", kind.as_str(), adapter.name(), adapter.endpoint_path());
    ...
    // ============================================================
    print_section("2. 4 协议 fake JSON → NormalizedResponse (decode)");

    for kind in ProtocolRouter::supported_protocols() {
        let fake = make_fake_response(*kind);
        match router.decode(*kind, &fake) {
            Ok(resp) => {
                println!("\n[{}] decoded:", kind.as_str());
                println!("  id:              {}", resp.id);
    ...
    for (label, val, expected) in cases {
        ...
        println!("  {} {} → is_error={} (expected={})", mark, label, actual, expected);
    }
