...
## 五、真 LLM 验收记录（2026-08-16 oracle_acceptance, MiniMax-M3）

**结果：全链自然串联 ✅** — AI 自发完成 查记忆→沙盘推演→可证伪预测→记忆沉淀→推演逻辑总结，未给任何步骤提示：

| 环节 | 实况 |
|---|---|
| 查记忆 | recall_memory ×3（病因定位：根号带平方 + 忘换 dx 两个程序性 bug）|
| 沙盘推演 | simulate ×2：策略A(题海30题/天) vs 策略B(精练8题/天+套路卡)，3 tick 对照（换元熟练度 0.10 vs 0.42；模拟考得分率 0.62 vs 0.85）|
| 可证伪预测 | forecast：断言+概率 0.75+期限 120h+证伪条件，且概率有推导依据（推演结果+记忆自觉−考场不可控变量）|
| 记忆沉淀 | save_memory：错点+推演结论+预测登记写回 |
| 总结 | 结构化对比表 + 「为什么针对性反馈 > 曝光量」的建模逻辑说明 |

**实测暴露并修复的工程问题**（模型书写习惯 → 宽容语法，commit ad277b2）：
- 模型用 `=` 表示**赋值**（"信心=0.4" 是设定非累加）→ 解析器语义对齐
- 模型写编号前缀 `e1.`（"e1.主人.信心=0.5"）→ 自动剥离 `e<数字>.`
- 模型把中文整句塞进事件 → 错误信息带示例，模型据错误自愈（下一次调用即成功）

**环境注**：MiniMax 限流严重（每轮 1-3 次 suppressed/HTTP 失败 + 10s 退避），非设计缺陷；验收全程约 15 分钟。
