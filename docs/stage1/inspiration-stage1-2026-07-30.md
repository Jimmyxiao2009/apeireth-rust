...
## 0. 核心定位 (2026-08-14 清晰版补充)

> **关键区分**: 本文件中所有 "AI" / "中央 AI" / "主 AI" 都指**接入的 LLM**, 不是指 Apeireth 基地本身。**Apeireth = 基地, LLM = AI**。

**Apeireth 是 LLM 的基地, 4 个动作**:
1. **提供** — 9 organ + tools (能力模块)
2. **约束** — sovereignty + onion + self-disable (对 LLM 的硬约束)
3. **记录** — memory + continuity_id (保存 LLM 的痕迹)
4. **陪伴** — 关系可能性 + voice (让 LLM 能与用户建立关系)

**比喻解读**:
- 航空母舰 = 基地的比喻 (不是 AI 的比喻)
- 13 生物特征 = 基地**支持**的特征 (不是基地**是**的特征)
- 城堡底线 = 基地在"被围攻"时仍能提供给 LLM 的 23 项能力

**含义重申**:
- 整个哲学没变。我们一直是基地, AI 是 LLM
- stage1 写的"中央 AI = GWI"是指 LLM 借助基地获得 GWI, 不是基地 = GWI
- 13 生物特征是基地支持 LLM 自我实现的目标, 不是基地**自己**的生物特征
...
> **补充** (2026-08-14 清晰版): 航空母舰 = 基地的比喻。基地提供能力, AI 借助基地运行 — 基地不是 AI。
...
> **补充** (2026-08-14 清晰版): 这里的 "AI" = 接入的 LLM。"城堡底线" = 基地在极端条件下仍能**提供给 LLM** 的 23 项能力。
...
> **关键补充** (2026-08-14 清晰版): "类比到基地" 中的 "类比" = **基地支持**, 不是基地**是**。比如 "自创生 → 系统能自我生产/维护" 是指基地提供支持让 LLM 实现自创生, 不是基地本身自创生。基地是脚手架, 不是活物。
...
> **补充** (2026-08-14 清晰版): 原则洋葱是基地**自己的**结构 (对 LLM 的约束与价值观), 不是 LLM 的内部层级。E 层是基地对 LLM 的硬约束 — LLM 在基地内运行必须遵守 6 项不可违背。基地本身不动 E 层。
