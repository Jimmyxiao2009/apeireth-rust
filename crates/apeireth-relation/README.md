...
> Apeireth relation subsystem (A12) — 4 relation kinds + property graph + traversal + predicate query.
...
```rust
use apeireth_relations::{Relation, RelationKind, RelationRegistry};
...
```rust
use apeireth_relations::{GraphEdge, GraphNode, RelationGraph, RelationKind};
...
```rust
use apeireth_relations::{BfsIter, DfsIter, TraversalDirection, shortest_path};
...
```rust
use apeireth_relations::query::{NodeQuery, EdgeQuery, count_by_kind};
...
Run with `cargo test -p apeireth-relation`.
...
- `cargo run -p apeireth-relation --example relation_demo` — existing flat demo
- `cargo run -p apeireth-relation --example graph_demo` — R154 graph + traversal + query
