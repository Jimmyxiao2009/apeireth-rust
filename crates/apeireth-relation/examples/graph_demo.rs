...
//!
//! Demonstrates the R154 graph / traversal / query modules of apeireth-relation.
...
fn main() {
    ...
    if let Some(path) = apeireth_relations::shortest_path(&graph, "memory", "reflection") {
...
fn graph_bfs(graph: &RelationGraph, root: &str, max_depth: usize) -> Vec<(String, usize)> {
    use apeireth_relations::BfsIter;
...
fn classify_pair_to_decision(a: &str, b: &str) -> apeireth_relations::RelationDecision {
    if a == b {
        apeireth_relations::RelationDecision::AEqualsB
    } else {
        apeireth_relations::RelationDecision::Default
    }
}
