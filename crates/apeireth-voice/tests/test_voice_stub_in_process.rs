...
fn fixture_1_compile_time_constants_match() {
    assert_eq!(
        ...
        "voice schema version must be '1'"
    );
    assert_eq!(
        PLATFORM_NAME, "apeireth",
        "platform name must be 'apeireth' (K-1 品牌一致)"
    );
    assert!(
        STUB_MODE,
        "STUB_MODE must be true until R20 stage 3 (K-1 强校验守门)"
    );
    assert_eq!(
        VOICE_SAMPLE_RATE_HZ, 16000,
        "Porcupine official sample rate"
    );
    assert_eq!(VOICE_FRAME_LENGTH, 512, "Porcupine official frame length");
    assert_eq!(
        VOICE_DEFAULT_KEYWORD, "apeireth",
        "default wake word must be 'apeireth' (K-1 品牌一致)"
    );
...
fn fixture_2_wake_word_type_has_5_variants() {
    assert_eq!(
        SUPPORTED_WAKE_WORDS.len(),
        5,
        "K-1 强校验: 必须 5 个 WakeWordType"
    );
...
fn fixture_3_tool_whitelist_has_9_tools() {
    assert_eq!(
        TOOL_WHITELIST.len(),
        9,
        "K-1 强校验: 必须 9 个工具 (8 商业版 + 1 stub_status)"
    );
...
fn fixture_4_k1_keywords_and_stub_mode() {
    ...
    assert!(
        source.contains("wake_word"),
        "must-do: 源码必须出现 'wake_word' (K-1 字样 #4)"
    );
    assert!(
...
async fn fixture_5_8_stub_tools_return_not_implemented() {
    ...
    assert!(
        ...
        "wake_word_detect must return NotImplemented, got {:?}",
        r1
    );
    ...
    assert!(
        ...
        "record_audio must return NotImplemented, got {:?}",
        r2
    );
    ...
    assert!(
        r5.is_ok(),
        "list_keywords must succeed (compile-time constant), got {:?}",
        r5
    );
    ...
    assert!(
        ...
        "load_model must return NotImplemented, got {:?}",
        r6
    );
    ...
    assert!(
        ...
        "unload_model must return NotImplemented, got {:?}",
        r7
    );
    ...
    assert!(
        ...
        "audio_stream must return NotImplemented, got {:?}",
        r8
    );
}
...
fn extra_1_default_keyword_is_apeireth() {
    assert_eq!(
        VOICE_DEFAULT_KEYWORD, "apeireth",
        "默认唤醒词必须 'apeireth' (1:1 翻译 v0.9.21 品牌一致)"
    );
    assert_eq!(
