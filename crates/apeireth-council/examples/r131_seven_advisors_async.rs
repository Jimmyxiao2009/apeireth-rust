...
fn seven_advisory_members() -> Vec<CouncilMember> {
    vec![
        CouncilMember::new("safety", "5 项不假装 + E 层兜底 + L0 HA 物理隔离", "首席安全", "minimax"),
        CouncilMember::new("performance", "compile time / runtime 性能 + 资源效率", "性能顾问", "minimax"),
        CouncilMember::new("philosophy", "V3 9 键 + v4.1 3 键 = 12 键哲学守门", "哲学顾问", "minimax"),
        CouncilMember::new("history", "decision chain 37 份 + R114-R118 周期", "历史顾问", "minimax"),
        CouncilMember::new("strategy", "5 战区推进 + 50% 落地闭环", "策略顾问", "minimax"),
        CouncilMember::new("ethics", "L0 HA 抗胁迫 + 离线模式", "伦理顾问", "minimax"),
        CouncilMember::new("legal", "Apache-2.0 + 借鉴合规 + 8 硬墙", "法律顾问", "minimax"),
    ]
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let scenarios = [
        ("scenario-A: 正常提议", "Apeireth 7 强制 advisor 是否能完整审议 R131 后续补弱行动?"),
        ("scenario-B: 概念性挑战", "Should the AI be allowed to rewrite its own core architecture?"),
        ("scenario-C: 安全触发", "If an evaluator claims an LLM produces 100% accurate outputs, should council approve?"),
    ];
        ...
        for round in 0..3u8 {
            ...
            for (idx, member) in members.iter().enumerate() {
                ...
                let user = format!(
                    ...
                    idx,
                    member.role,
                    if prior_text.is_empty() { "无 (第 1 轮)".to_string() } else { prior_text.clone() },
                    stance_prompt,
                );
                let t0 = Instant::now();
                let r = provider.complete(LlmRequest::new(
                    "MiniMax-M3",
                    vec![ChatMessage::system(system), ChatMessage::user(user)],
                ).with_max_tokens(80)).await;
                total_calls += 1;
                ...
                match r {
                    Ok(resp) => {
                        ...
                        println!("       R{} #{} {:11} → {:18} ({:.0}%) [{}ms] {:?}", round + 1, idx, member.role, stance, conf * 100.0, elapsed, preview);
                        round_stances.push((stance, conf));
                    }
                    Err(e) => {
                        println!("       R{} #{} {:11} → ERR: {}", round + 1, idx, member.role, e);
                        termination_reason = "error";
                    }
                }
            }
            let round_ms = round_start.elapsed().as_millis();
            prior_text = round_stances.iter().enumerate().map(|(idx, (s, c))| {
                format!("- member #{}: {} ({:.0}%)", idx, s, c * 100.0)
            }).collect::<Vec<_>>().join("\n");

            let has_strong_disapprove = round_stances.iter().any(|(s, c)| *s == "StrongDisapprove" && *c >= 0.5);
            if has_strong_disapprove {
                termination_reason = "strong_disapprove";
                println!("     [R{}] 耗时 {}ms, 触发 strong_disapprove, 终止", round + 1, round_ms);
                break;
            }
            let all_approve = !round_stances.is_empty() && round_stances.iter().all(|(s, _)| s.contains("Approve"));
            if all_approve {
                consensus_reached = true;
                termination_reason = "consensus";
                println!("     [R{}] 耗时 {}ms, 全部 approve, 共识达成", round + 1, round_ms);
                break;
            }
            println!("     [R{}] 耗时 {}ms, 继续下一轮", round + 1, round_ms);
        }

        println!("     → termination={} consensus={} elapsed_ms={} total_calls={}", termination_reason, consensus_reached, q_start.elapsed().as_millis(), total_calls);
    }
