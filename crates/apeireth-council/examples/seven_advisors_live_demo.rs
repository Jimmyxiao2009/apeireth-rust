...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let backend: Arc<dyn MockLlmProvider> = Arc::new(LlmAdvisorBackend::new(Arc::clone(&provider) as Arc<dyn LlmProvider>));
    ...
    for (i, (label, desc)) in scenarios.iter().enumerate() {
        ...
        println!("     rounds={} final_stance={:?} final_score={:.3} consensus={} elapsed_ms={}",
            verdict.rounds_run, verdict.final_stance, verdict.final_weighted_score,
            verdict.consensus_reached, elapsed);
        println!("     members ({}):", verdict.member_summaries.len());
        for m in &verdict.member_summaries {
            println!("       - {:15} provider={:10} -> {:?} ({:.0}%)",
                m.role, m.provider, m.final_stance, m.final_confidence * 100.0);
        }
    }
