...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let query = CouncilQuery::new(
        "r131-3-q",
        "Apeireth R132 应该优先做哪一项? deploy / design / test / fix?",
        now_ms(),
    );
    ...
    println!(
        "  baseline (no LLM, keyword evaluate): {}ms",
        pe_baseline_ms
    );
    ...
    for st in &plan {
        let prompt = format!(
            "你是 Apeireth {}. 评估子任务 '{}' 对 query '{}' 的可行性.\n{}",
            st.role, st.description, query.description, STANCE_PROMPT
        );
        let ((stance, conf), _text, ms) = llm_evaluate(
            &provider,
            "你是 Apeireth 项目 planner+executor 评估器",
            &prompt,
        )
        .await;
        llm_total += ms;
    ...
    let voter_specs = [
        (
            "voter-arch",
            "architect",
            "你是 Apeireth 主架构师, 评估技术风险",
        ),
        ("voter-qa", "qa", "你是 Apeireth QA, 评估测试覆盖"),
        (
            "voter-product",
            "product",
            "你是 Apeireth 产品, 评估用户价值",
        ),
    ...
    for (id, role, _sys) in voter_specs.iter() {
        ...
        let ((stance, conf), _text, _ms) =
            llm_evaluate(&provider, &format!("你是 Apeireth {}", role), &prompt).await;
        ...
        voters.push(Voter::new(
            ...
            *role,
            stance_enum,
            conf,
            format!("LLM voter: {}", stance),
        ));
    }
    let mut vm = VotingMode::new(voters).with_strategy(VotingStrategy::WeightedMajority);
    let v_verdict = vm.run(&query);
    println!(
        "  → aggregated_stance={:?} final_score={:.3} elapsed={}ms",
        v_verdict.report.aggregated_stance, v_verdict.report.weighted_score, v_verdict.elapsed_ms
    );
    println!("  → 4 voters:");
    ...
    let mut hm_verdict = CollaborationVerdict {
        ...
        report: apeireth_council::synthesis::SynthesisReport {
            weighted_score: 0.0,
            aggregated_stance: apeireth_council::advisor::Stance::new(
                apeireth_council::advisor::StanceKind::Neutral,
                String::from("init"),
            ),
            confidence: 0.0,
            dissenting: Vec::new(),
    ...
    for t in &sub_tasks {
        ...
        let ((stance, conf), _text, ms) =
            llm_evaluate(&provider, &format!("你是 Apeireth {}", t.sub_role), &prompt).await;
        println!(
            "  sub {} ({:11}): {:18} ({:.0}%) [{}ms]",
            t.task_id,
            t.sub_role,
    ...
    println!(
        "  → root verdict: aggregated_stance={:?} score={:.3} elapsed={}ms",
        h_verdict.report.aggregated_stance, h_verdict.report.weighted_score, h_verdict.elapsed_ms
    );
