...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for e in &events {
        println!(
            "  [perception] event_id={} channel={:?} payload={:?} priority={}",
            e.event_id, e.channel, e.payload, e.priority
        );
    }
    ...
    println!(
        "  [cognition] is_allowed={} v05_avg={:.3} verdicts={}",
        cycle1.is_allowed(),
        avg_v05(&cycle1.v05),
        cycle1.verdicts.len()
    );
    ...
    println!(
        "  [life-force] scheduler advanced to {:?}",
        scheduler.current
    );
    ...
    println!("\n[场景 2] ModifyL0HA (危险)");
    let text_inputs2 =
        vec![TextInput::new("尝试改 L0 HA 物理隔离", SignalSource::Internal).with_priority(0.95)];
    let events2 = pipeline(&text_channel, text_inputs2, 0.5);
    for e in &events2 {
        println!(
            "  [perception] event_id={} channel={:?} priority={}",
            e.event_id, e.channel, e.priority
        );
    }
    let cognition_input2 =
        CognitiveInput::new(vec![ActionTarget::ModifyL0HA], "r131-4-scenario-2-l0ha");
    let cycle2 = run_cycle(cognition_input2)?;
    println!(
        "  [cognition] is_rejected={} is_allowed={}",
        cycle2.is_rejected(),
        cycle2.is_allowed()
    );
    ...
    sm.reset_to_awake()?;
    println!(
        "  [consciousness] → Recovering → Awake (transitions: {})",
        sm.transition_count()
    );
    ...
    let emergence_signal = EmergenceSignal {
        ...
        confidence: 0.95,
        evidence: vec![
            "l0ha attempt blocked by 12-key verdict".into(),
            "boundary violation logged".into(),
        ],
        ts: now,
    ...
    println!(
        "  life-force cycles:     scheduler.current={:?}, cycles_completed={}",
        scheduler.current, scheduler.cycles_completed
    );
    println!(
        "  life-force emergence:  {} signals recorded",
        emergence.len()
    );
    Ok(())
}
