...
use apeireth_workflow::{Activity, ActivityInput, ActivityOutput, Workflow, WorkflowContext, WorkflowRunner, WorkflowResult};
use std::sync::Arc;
...
fn main() {
    ...
    let result = r.run("add", &serde_json::json!({"a": 10, "b": 20})).unwrap();
