...
use crate::consciousness_bridge::{apply_plutchik_to_life_force, plutchik_to_life_force_adjustment};
use crate::LifeForce;
...
fn r176_b2_01_basic_endurance_delta_clamped() {
    for b in &[
        ...
        let adj = plutchik_to_life_force_adjustment(&e);
        assert!(adj.endurance_delta >= -0.2 && adj.endurance_delta <= 0.2,
                "endurance_delta {} out of range for {:?}", adj.endurance_delta, b);
    }
}
...
fn r176_b2_02_advanced_endurance_delta_clamped() {
    for a in &[
        ...
        let adj = plutchik_to_life_force_adjustment(&e);
        assert!(adj.endurance_delta >= -0.2 && adj.endurance_delta <= 0.2,
                "endurance_delta {} out of range for {:?}", adj.endurance_delta, a);
    }
}
...
fn r176_b2_03_apply_returns_ok_for_valid_input() {
    ...
    for start in &[0.3, 0.5, 0.7] {
        for b in &[
            ...
            assert!(result.is_ok(), "apply failed for {:?} from {}: {:?}", b, start, result);
            assert!(life.endurance >= 0.0 && life.endurance <= 1.0,
                    "endurance {} out of range after apply for {:?} from {}", life.endurance, b, start);
        }
    }
}
...
fn r177_b2_03b_clamp_instead_of_err() {
    for b in &[
        PlutchikBasic::Joy, PlutchikBasic::Trust, PlutchikBasic::Anticipation,
    ] {
        let mut life = fresh_life_force();
        life.endurance = 1.0;
        let e = make_basic(*b);
        ...
        assert!(result.is_ok(), "expected Ok after clamp for {:?} from 1.0", b);
        assert_eq!(life.endurance, 1.0, "endurance should be clamped to 1.0 for {:?}", b);
    }
}
...
fn r177_b2_03c_clamp_zero_instead_of_err() {
    for b in &[
        PlutchikBasic::Fear, PlutchikBasic::Sadness, PlutchikBasic::Disgust,
    ] {
        let mut life = fresh_life_force();
        life.endurance = 0.0;
        let e = make_basic(*b);
        let result = apply_plutchik_to_life_force(&mut life, &e, 1000);
        assert!(result.is_ok(), "expected Ok after clamp for {:?} from 0.0", b);
        assert_eq!(life.endurance, 0.0, "endurance should be clamped to 0.0 for {:?}", b);
    }
}
...
fn r176_b2_04_trigger_reason_consistent() {
    for b in &[
        ...
        if adj.should_trigger_reflection {
            assert!(adj.reflection_reason.is_some(), "triggered but no reason for {:?}", b);
        } else {
            assert!(adj.reflection_reason.is_none(), "not triggered but reason set for {:?}", b);
        }
    }
}
