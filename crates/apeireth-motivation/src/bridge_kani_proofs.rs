...
use crate::consciousness_bridge::{
    apply_plutchik_to_external_drive, apply_plutchik_to_internal_drive, plutchik_to_motivation_adjustment,
};
use crate::life_force_bridge::{apply_life_force_to_external_drive, apply_life_force_to_internal_drive, life_force_to_motivation_adjustment};
...
fn fresh_internal_drive() -> InternalDrive {
    InternalDrive { label: "test".into(), intensity: 0.5 }
}

fn fresh_external_drive() -> ExternalDrive {
    ExternalDrive { label: "test".into(), intensity: 0.5 }
}
...
fn r176_b3_03_apply_internal_drive_safe() {
    for b in &[
        ...
        for start in &[0.3, 0.5, 0.7] {
            ...
            assert!(new_intensity >= 0.0 && new_intensity <= 1.0,
                    "intensity {} out of range for {:?} from {}", new_intensity, b, start);
        }
    }
}
...
fn r176_b3_05_sgi_intensity_valid() {
    for b in &[
        ...
        if let Some(sgi) = &adj.sgi_suggestion {
            assert!(sgi.intensity >= 0.0 && sgi.intensity <= 1.0,
                    "sgi intensity {} out of range for {:?}", sgi.intensity, b);
        }
    }
}
...
fn r176_b6_01_life_force_adjustment_valid() {
    ...
    for endurance in &[0.0, 0.25, 0.5, 0.75, 1.0] {
        ...
        assert!(adj.drive_intensity_multiplier >= 0.3 && adj.drive_intensity_multiplier <= 1.5,
                "drive_intensity_multiplier {} out of range for endurance {}", adj.drive_intensity_multiplier, endurance);
    }
}
...
fn r176_b6_02_apply_life_force_to_internal_safe() {
    for endurance in &[0.3, 0.5, 0.7] {
        ...
        assert!(new_intensity >= 0.0 && new_intensity <= 1.0,
                "intensity {} out of range for endurance {}", new_intensity, endurance);
    }
}
