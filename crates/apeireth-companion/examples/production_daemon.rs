...
use apeireth_companion::daemon::{
    open_memory_store, CompanionDaemon, CompanionDelivery, ConsoleSink, ThrottledUtterance,
    UtteranceGenerator,
};
...
use apeireth_companion::{
    tone_hint, AwakeCompanion, Bond, BondStage, ConstitutionLlm, LlmJudicator,
};
...
impl MiniMaxConstitutionLlm {
    pub fn new(api_key: String) -> Result<Self, String> {
        Ok(Self {
            pipeline: Arc::new(
                build_pipeline(BASE_URL.to_string(), Some(api_key)).map_err(|e| e.clone())?,
            ),
        })
    }
}
    ...
    fn apply(&self, _call: &ParsedToolCall, r: &ExecutionResult) -> ExecutionResult {
        if r.success {
            ...
            if let Some(o) = out.as_object_mut() {
                o.insert("traced".into(), json!(true));
            }
            ExecutionResult {
...
async fn main() {
    ...
    let _awake = AwakeCompanion::new(bond.clone(), Boundaries::default()).with_config(config);
    let tone = tone_hint(&bond);
    println!("[1] 记忆+器官就绪 · 语调: {tone}");

    // 2. ToolBridge 全增强
    let judge = Arc::new(LlmJudicator::new(Arc::new(
        MiniMaxConstitutionLlm::new(key.clone()).unwrap(),
    )));
    let worker = option_env!("CARGO_BIN_EXE_exec_worker")
        .unwrap_or("exec_worker")
        .to_string();
    let bridge = Arc::new(
    ...
    println!(
        "[3] SessionLog + ContinuationStore + Goal 就绪 (目标: {}, cid: {cid})",
        g.objective
    );
    ...
    let mut daemon = CompanionDaemon::new(
        ...
        CompanionDelivery::new(
            ThrottledUtterance::new(
                TonalUtterance::new(key.clone(), tone).unwrap(),
    ...
    for d in 1..=7i64 {
        daemon.awake.observe_interaction(
            now - chrono::Duration::days(d) + chrono::Duration::minutes(d % 3),
        );
    }
    let hint = None;
    ...
    if let Some(i) = init {
        ...
        slog.append(
            "user",
            json!({"content": format!("(系统: 主动问候 {})", i.action.id())}),
        )
        .unwrap();
    } else {
    ...
    let mut messages: Vec<Value> = vec![
        json!({"role": "user", "content": format!("任务: 主人高数换元法常出错。1. recall_memory 查他记忆 2. 用 save_memory 存一条『换元必换 dx』的提醒 3. FileOperator 写错题笔记到 {} (80字内) 4. 汇报", target.to_string_lossy())}),
    ];
    let mut turn: u64 = 1;
    ...
    for _ in 0..6 {
        ...
        slog.append("user", json!({"content": format!("(轮 {turn})")}))
            .unwrap();
        let req = OpenAiChatRequest {
            model: MODEL.to_string(),
            messages: messages
        ...
        slog.append("assistant", json!({"content": content, "tool_calls": tcs}))
            .unwrap();
        ...
        for tc in &tcs {
            ...
            slog.append(
                "tool",
                json!({"tool_call_id": id, "result": r.output.clone()}),
            )
            .unwrap();
