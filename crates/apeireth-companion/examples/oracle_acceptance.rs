...
async fn chat_once(pipeline: &Arc<Pipeline>, req: &OpenAiChatRequest, label: &str) -> Option<(String, Vec<Value>)> {
    for attempt in 0..5 {
        match dispatch(pipeline, ProtocolKind::OpenAiChat, openai_chat_to_normalized(req)).await {
            ...
            Err(e) => { eprintln!("  [管线] {label} 第{}次失败: {e}, 10s 后重试", attempt + 1); tokio::time::sleep(Duration::from_secs(10)).await; }
        }
    }
    None
}
...
async fn main() {
    ...
    // 种子记忆
    for (i, c) in ["高数: 换元法常错, 根号里带平方的", "高数: 换元后忘记换 dx", "周五高数期中考试"].iter().enumerate() {
    ...
    for _ in 0..8 {
        let req = OpenAiChatRequest {
            ...
            temperature: Some(0.5), max_tokens: Some(1500), stream: false, stop: None,
            tools: Some(tools.as_array().cloned().unwrap_or_default()), tool_choice: Some(json!("auto")),
        };
        let Some((content, tcs)) = chat_once(&pipeline, &req, "融合轮").await else { break; };
        if tcs.is_empty() {
        ...
        for tc in &tcs {
            ...
            println!("  他调用 {name}: {}", body.chars().take(110).collect::<String>());
    ...
    println!("查记忆 {} · 沙盘推演 {} · 可证伪预测 {} · 记忆沉淀 {}", memory_used, simulate_used, forecast_used, save_used);
    let fusion = memory_used && simulate_used && forecast_used;
    println!("自然融合(查→推→测): {}", if fusion { "✅ 全链串联" } else { "⚠️ 部分串联" });
