...
use apeireth_companion::{
    BondStage, Companion, MilestoneKind, MilestonePayload, PartnerId, PartnerPreferences,
};
...
async fn main() {
    ...
    let partner = companion
        .register_partner(id, "主人".to_string(), prefs)
        .await
        .unwrap();
    println!(
    ...
    let m1 = companion
        .record_milestone(
            ...
            MilestonePayload::Text("第一次对话".into()),
        )
        .await
        .unwrap();
    ...
    let m2 = companion
        .record_milestone(
            id,
            MilestoneKind::FirstEmotion,
            MilestonePayload::Text("首次表达情绪".into()),
        )
        .await
        .unwrap();
    ...
    partner
        .bond_mut()
        .apply_emotion(0.6, 0.7, 0.1, 0.3, 0.0, 0.0, 0.0, 0.5);
    println!(
        "[+] emotion applied: {:?}",
        partner.bond().character().serialize()
    );
    ...
    for entry in timeline.iter() {
        println!(
            "    - {} @ {}: {:?}",
            entry.milestone.kind().label(),
            entry.at,
            entry.milestone.payload()
        );
    }
