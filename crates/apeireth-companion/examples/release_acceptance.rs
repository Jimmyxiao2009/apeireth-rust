...
async fn main() {
    ...
    SuiteCatalog::builtin()
        .install(&bridge, "education-suite")
        .unwrap();
    bridge.packs.grant(PermissionPack::timed(
        "验收包",
        vec!["FileOperator".into()],
        24,
        Some(10),
    ));
    ...
    let sys = format!(
        "你是阿佩瑞斯, 诚实、有记忆的伙伴, 住在 Apeireth 基地。{}",
    ...
    for _ in 0..6 {
        ...
        let Some((content, tcs)) = chat_once(&pipeline, &req, "提案轮").await else {
            break;
        };
        if tcs.is_empty() {
    ...
    let pending = reg
        .list(Some(
            apeireth_companion::capability::CapabilityStatus::Pending,
        ))
        .unwrap();
    let cap = &pending[0];
    ...
    println!(
        "[catalog] 动态自述包含演化能力: {}",
        dynamic.contains("你已演化的能力")
    );
    if !active.is_empty() {
        println!(
            "  · [{}] {}: {}",
            active[0].kind.label_zh(),
            active[0].name,
    ...
    for _ in 0..5 {
        let req = OpenAiChatRequest {
            ...
            stream: false,
            stop: None,
            tools: Some(tools2.as_array().cloned().unwrap_or_default()),
            tool_choice: Some(json!("auto")),
        };
        let Some((content, tcs)) = chat_once(&pipeline, &req, "干活轮").await else {
            break;
        };
        if tcs.is_empty() {
        ...
        for tc in &tcs {
            ...
            println!(
                "  调用 {name}: {}",
                body.chars().take(70).collect::<String>()
            );
