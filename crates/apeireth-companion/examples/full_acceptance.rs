...
use apeireth_companion::daemon::{requires_llm_review, Judicator, UtteranceGenerator};
...
use apeireth_companion::{AwakeCompanion, Bond, BondStage, ContextSource};
...
// ============================================================
// 宪法评审 (demo 简化 — §6.2 待接真 LLM 评审, 诚实标注)
// ============================================================
...
impl Judicator for ConstitutionJudicator {
    async fn judge(&self, action: &str) -> Result<bool, String> {
        // demo 简化: 本地关键词网 (0 token), 写学习笔记 = ALLOW
...
async fn main() {
    ...
    println!("[A] 全器官(AwakeCompanion) + 安全 + 工具桥 就绪\n");
    ...
    for round in 1..=5 {
        ...
        for tc in &tcs {
            ...
            if requires_llm_review(risk) {
                ...
                match ConstitutionJudicator.judge(&desc).await {
                    Ok(true) => {}
                    Ok(false) => {
