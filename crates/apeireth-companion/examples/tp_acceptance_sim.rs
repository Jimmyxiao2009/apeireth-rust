...
use apeireth_team_lead::{ApprovalBridge, ApprovalResponse as WireApprovalResponse, InProcessBridge};
...
async fn s1_handoff() -> Result<(), String> {
    ...
    check("工具注册", installed >= 2, &format!("install_handoff_tools 返回 {installed}"))?;
    ...
    check("orchestrator 解析委托调用", calls.len() == 1 && calls[0].tool_name == "transfer_to_researcher", "解析出 1 个 transfer 调用")?;

    let executor = ToolExecutor::with_timeout(Arc::clone(&registry), 10_000);
    let result = executor.execute(&calls[0]).await;
    check("委托执行成功", result.success, &format!("duration_ms={}", result.duration_ms))?;
    check("目标 agent 收到委托 (OnHandoff)", delivered.load(Ordering::SeqCst) == 1, "回调触发 1 次")?;
    ...
    check("禁用目标委托被拒", !result2.success, "set_enabled(false) 后 transfer 应失败")?;
    Ok(())
}
...
async fn s2_schema_guardrail() -> Result<(), String> {
    ...
    validate(&schema, &json!({"op": "read", "path": "a.txt", "content": "hi"}))
        .map_err(|e| format!("合法输出被拒: {e:?}"))?;
    check("合法输出通过", true, "op/path/content 全类型正确")?;

    let r_missing = validate(&schema, &json!({"op": "read", "path": "a.txt"}));
    check("缺字段被拒", r_missing.is_err(), format!("缺失 content → {:?}", r_missing.err()))?;

    let r_type = validate(&schema, &json!({"op": "read", "path": "a.txt", "content": 42}));
    ...
    check("pre_call_guard 路径穿越拒绝", bad.is_err(), format!("`../secret.txt` → {bad:?}"))?;
    let bad2 = pre_call_guard("ShellExec", &inject_args);
    check("pre_call_guard 命令注入拒绝", bad2.is_err(), format!("`; rm -rf /` → {bad2:?}"))?;
    ...
    check("post_call_tripwire 凭据泄漏触发", bad_out.is_some(), "输出含 AWS AKIA key 应触发 tripwire")?;
    let ok_out = post_call_tripwire("FileOperator", &json!({"op": "read", "path": "x", "status": 0}));
    check("post_call_tripwire 干净输出不触发", ok_out.is_none(), "无凭据应放行")?;
    ...
    check("executor 注入 schema map", executor.schemas().get("FileOperator").is_some(), "with_schema_map 生效")?;
...
fn s3_calibration_forecast() -> Result<(), String> {
    ...
    check("Brier 分数", (brier - 0.21).abs() < 0.001, &format!("brier={brier:.4} (期望≈0.21)"))?;
    ...
    check("集合预报聚合", (0.0..=1.0).contains(&ens.aggregate_prediction), &format!("聚合预报={:.3}", ens.aggregate_prediction))?;
    let as_obs = ens.as_observation(1.0);
    check("集合预报 → 观测 (喂回校准)", as_obs.forecast == ens.aggregate_prediction, "as_observation 保留 forecast")?;

    // 预测市场: 下单 → 收据 → 聚合信念
    let mut market = PredictionMarket::new(MarketConfig::default());
    let prices = market.prices();
    check("市场报价", prices.len() >= 2, &format!("outcome 数={}", prices.len()))?;
    let receipt = market.execute_buy(0, 10.0).map_err(|e| format!("下单失败: {e:?}"))?;
    check("下单收据", receipt.shares > 0.0 && receipt.cost >= 0.0, &format!("shares={:.2} cost={:.2}", receipt.shares, receipt.cost))?;
    let belief = market.aggregate_belief(0);
    check("聚合信念", (0.0..=1.0).contains(&belief), &format!("belief[0]={belief:.3}"))?;

    // Critic: 审阅同一段历史
    let critic = Critic::default_critic();
    let cr = critic.critique(&obs);
    println!("    critic: severity={:.4} brier_est={:.4} ece={:.4} action={:?}", cr.severity, cr.brier_estimate, cr.expected_calibration_error, cr.recommended_action);
    check("Critic 审阅产出 (推荐动作非空)", !format!("{:?}", cr.recommended_action).is_empty(), "有推荐动作")?;
    Ok(())
}
...
fn s4_approval_bridge() -> Result<(), String> {
    ...
    check("请求落库 pending", pending.len() == 1, "1 条 ShellExec 待批")?;
    ...
    check("orchestrator 收到 approved 响应", resp.is_some(), "bridge 双向同步生效")?;
    Ok(())
}
...
async fn main() {
    ...
    for (name, fut) in [
        ("S1 handoff", Box::pin(s1_handoff()) as std::pin::Pin<Box<dyn std::future::Future<Output = Result<(), String>>>>),
    ...
    match (s3_calibration_forecast(), s4_approval_bridge()) {
        (Ok(()), Ok(())) => println!("  ✓ S3 calibration_forecast PASS\n  ✓ S4 approval_bridge PASS"),
        (a, b) => {
            if let Err(e) = a {
