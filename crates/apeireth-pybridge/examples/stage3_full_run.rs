...
fn main() {
    ...
    println!("  asi_count: {}, r11_count: {}", e2e.asi_module_count, e2e.r11_module_count);
    println!("  pool: max_idle={} idle_timeout={}s", e2e.pool_max_idle, e2e.pool_idle_timeout_secs);
    println!("  categories_in_use: {}, ceiling_critical: {}", e2e.categories_in_use, e2e.ceiling_critical_modules.len());
    println!("  modules_in_scope: {:?}", e2e.modules_in_scope);
    println!("  e2e_ok: {} (python_ext: {}, python_available: {})\n",
    ...
    println!("  total_wallclock: {:.2}ms", bench.total_wallclock.as_micros() as f64 / 1000.0);
    ...
    println!("  probes: {}/{} OK", xmod.probe_results.iter().filter(|p| p.ok).count(), xmod.probe_results.len());
    for p in &xmod.probe_results {
        println!("  - {:?}: {} ({})", p.kind, if p.ok { "OK" } else { "FAIL" }, p.description);
    }
    println!();
    ...
    println!("    B2 (workspace.version 1.2.0 0 改):     {}", if walls.b2_workspace_version_unchanged { "PASS" } else { "FAIL" });
    println!("    A1 (R11 baseline 0.8682/0.8532/0.9063): {}", if walls.a1_baseline_locked { "PASS" } else { "FAIL" });
    println!("    B1 (24 LOCKED 入口签名 0 改):           {}", if walls.b1_24_locked_unchanged { "PASS" } else { "FAIL" });
    println!("    B5 (8 哲学锚):                          {}", if walls.b5_8_philosophical_anchors { "PASS" } else { "FAIL" });
    println!("    B3 (V0.5 30 维):                        {}", if walls.b3_30_dimensions { "PASS" } else { "FAIL" });
    println!("    B4 (6 重守门 v7):                       {}", if walls.b4_6_gates_v7 { "PASS" } else { "FAIL" });
    println!("    A3 (12 键 + PHL-07 = 13 键):            {}", if walls.a3_13_keys { "PASS" } else { "FAIL" });
    println!("    C1 (0 主动 commit):                     {}", if walls.c1_no_commit { "PASS" } else { "FAIL" });
    println!("    C2 (0 装 PASS 严守):                    {}", if walls.c2_no_fake_pass { "PASS" } else { "FAIL" });
    println!("    C3 (升 6 重 v7):                        {}", if walls.c3_6_gates_v7 { "PASS" } else { "FAIL" });
    println!();
