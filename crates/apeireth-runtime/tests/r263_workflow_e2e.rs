...
use apeireth_workflow::{
    Activity, ActivityInput, ActivityOutput, Workflow, WorkflowContext,
    WorkflowResult, WorkflowRunner, WorkflowWorker,
};
...
async fn r263_e2e_workflow_dispatched_via_runtime() {
    ...
    let task_id = runtime.dispatch_async_task(
        "workflow.double",
        r#"{"n":21}"#,
    ).await;

    let rec = runtime.task_store.wait_for_completion(task_id, Duration::from_secs(10)).await
    ...
    assert!(hist.len() >= 4, "expected at least 4 events, got {}", hist.len());
    assert_eq!(hist[0].kind, apeireth_workflow::EventKind::WorkflowStarted);
    assert_eq!(hist.last().unwrap().kind, apeireth_workflow::EventKind::WorkflowCompleted);
}
...
async fn r263_e2e_workflow_error_marks_failed() {
    ...
    let task_id = runtime.dispatch_async_task("workflow.double", r#"{"n":1}"#).await;
    let rec = runtime.task_store.wait_for_completion(task_id, Duration::from_secs(10)).await
        .expect("wait_for_completion");
    assert_eq!(rec.status, TaskStatus::Failed, "expected Failed, got {:?}", rec.status);
