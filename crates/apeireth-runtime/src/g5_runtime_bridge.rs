...
pub struct RuntimeDispatchStage;
...
#[derive(Debug, Clone)]
pub struct RuntimeNormalizeStage {
    pub max_payload_len: usize,
}
impl RuntimeNormalizeStage {
...
impl Stage<PipelineMessage, PipelineMessage> for RuntimeNormalizeStage {
    ...
    fn process(
        ...
        let mut m = input;
        if m.payload.len() > self.max_payload_len {
            let mut idx = self.max_payload_len;
...
pub struct RuntimePolicyStage {
    ...
    pub state: std::sync::Arc<ConcurrencyState>,
}
impl RuntimePolicyStage {
    pub fn new(config: PolicyConfig, state: std::sync::Arc<ConcurrencyState>) -> Self {
        Self { config, state }
    }
}
impl Stage<PipelineMessage, PipelineMessage> for RuntimePolicyStage {
    ...
    fn process(
        ...
        let m = input;
        if !self.state.try_acquire(self.config.max_concurrent) {
            return Err(apeireth_pipeline_g5::PipelineError::Stage {
                kind: StageKind::Policy,
                source: "concurrency cap exceeded".into(),
            });
        }
        Ok(m)
    }
...
pub struct RuntimeReliabilityStage {
    pub state: std::sync::Arc<ReliabilityState>,
}
impl RuntimeReliabilityStage {
    ...
    fn process(
        ...
        if self.state.is_suppressed(&fp) {
            ...
            return Err(apeireth_pipeline_g5::PipelineError::Stage {
                ...
                source: format!("task {} suppressed", fp).into(),
            });
        }
        m.attempt += 1;
        Ok(m)
...
pub struct RuntimeThrottleStage {
    pub state: std::sync::Arc<Mutex<HashMap<Instant, usize>>>,
    pub config: ThrottleConfig,
}
impl RuntimeThrottleStage {
...
impl Stage<PipelineMessage, PipelineMessage> for RuntimeThrottleStage {
    ...
    fn process(
        ...
        let now = Instant::now();
        let cutoff = now - self.config.tick_interval;
        map.retain(|t, _| *t > cutoff);
        let count: usize = map.values().sum();
        if count >= self.config.max_per_tick {
            return Err(apeireth_pipeline_g5::PipelineError::Stage {
                kind: StageKind::Throttle,
                source: "tick rate exceeded".into(),
            });
        }
...
pub struct RuntimePipelineBuilder {
    ...
    concurrency_state: std::sync::Arc<ConcurrencyState>,
}
impl RuntimePipelineBuilder {
    pub fn new(name: impl Into<String>) -> Self {
        Self {
            name: name.into(),
            normalize_max: 16 * 1024,
            ...
            concurrency_state: std::sync::Arc::new(ConcurrencyState::new()),
        }
    }
    pub fn with_normalize_max(mut self, max: usize) -> Self {
        self.normalize_max = max;
        self
    }
    pub fn with_policy(mut self, c: PolicyConfig) -> Self {
        self.policy_config = c;
        self
    }
    pub fn with_throttle(mut self, c: ThrottleConfig) -> Self {
        self.throttle_config = c;
        self
    }
    pub fn with_reliability(mut self, s: std::sync::Arc<ReliabilityState>) -> Self {
        self.reliability_state = s;
        self
    }
    pub fn build(self) -> Pipeline<RuntimePipeline, PipelineMessage, PipelineMessage> {
        let config = PipelineConfig::new(self.name, "RuntimePipeline");
        Pipeline::new(config)
            .with_stage(RuntimeDispatchStage)
            .with_stage(RuntimeNormalizeStage::new(self.normalize_max))
            .with_stage(RuntimePolicyStage::new(
                self.policy_config,
                self.concurrency_state,
            ))
            .with_stage(RuntimeReliabilityStage::new(self.reliability_state))
            .with_stage(RuntimeThrottleStage::new(self.throttle_config))
    }
}
...
mod tests {
    ...
    fn dispatch_defaults_kind() {
        let s = RuntimeDispatchStage;
        let m = PipelineMessage::new("", "task");
        let o = s.process(m).unwrap();
        assert_eq!(o.kind, "task-pending");
    }
    #[test]
    fn dispatch_preserves_kind() {
        let s = RuntimeDispatchStage;
        let m = PipelineMessage::new("task-running", "x");
        let o = s.process(m).unwrap();
        assert_eq!(o.kind, "task-running");
    }
    #[test]
    fn normalize_truncates() {
        let s = RuntimeNormalizeStage::new(10);
        let m = PipelineMessage::new("task", "a".repeat(100));
        let o = s.process(m).unwrap();
        assert!(o.payload.starts_with(&"a".repeat(10)));
    }
    #[test]
    fn policy_caps_concurrency() {
        let state = std::sync::Arc::new(ConcurrencyState::new());
        let s = RuntimePolicyStage::new(PolicyConfig { max_concurrent: 1 }, state.clone());
        assert!(s.process(PipelineMessage::new("task", "a")).is_ok());
        assert!(s.process(PipelineMessage::new("task", "b")).is_err());
        state.release();
        assert!(s.process(PipelineMessage::new("task", "c")).is_ok());
    }
    #[test]
    fn reliability_suppresses_repeat() {
        let s = RuntimeReliabilityStage::new(std::sync::Arc::new(ReliabilityState::new()));
        let m = PipelineMessage::new("task", "unique-r-1");
    ...
    fn throttle_blocks_over_limit() {
        let s = RuntimeThrottleStage::new(ThrottleConfig {
            max_per_tick: 1,
            tick_interval: Duration::from_millis(100),
        });
        assert!(s.process(PipelineMessage::new("task", "a")).is_ok());
    ...
    fn full_pipeline_runs() {
        let p = RuntimePipelineBuilder::new("rt")
            .with_policy(PolicyConfig { max_concurrent: 8 })
            .build();
        let r = p.run(PipelineMessage::new("task", "classify-request"));
        assert!(r.is_ok(), "should pass: {:?}", r.err());
    }
    #[test]
    fn pipeline_stage_order() {
        let p = RuntimePipelineBuilder::new("rt-order").build();
        let k = p.stage_kinds();
        assert_eq!(k.len(), 5);
