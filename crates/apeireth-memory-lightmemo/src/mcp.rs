...
impl LightMemoTool {
    ...
    pub fn all() -> &'static [LightMemoTool] {
        &[LightMemoTool::MemoryAdd, LightMemoTool::MemoryGet, LightMemoTool::MemorySearch, LightMemoTool::DecayCheck, LightMemoTool::DreamCycle]
    }
}
        ...
        match req.method.as_str() {
            ...
            "tools/call" => {
                ...
                let summary = match tool {
                    "memory_add" => format!("add content={:?}", args.get("content").and_then(|v| v.as_str()).unwrap_or("")),
                    "memory_get" => format!("get id={:?}", args.get("id").and_then(|v| v.as_str()).unwrap_or("")),
                    "memory_search" => format!("search query={:?}", args.get("query").and_then(|v| v.as_str()).unwrap_or("")),
                    "decay_check" => format!("decay id={:?}", args.get("id").and_then(|v| v.as_str()).unwrap_or("")),
...
impl Default for LightMemoMcp { fn default() -> Self { Self::new() } }

#[cfg(test)]
mod tests {
    use super::*;
    #[test]
    fn tool_count() { assert_eq!(LIGHTMEMO_MCP_TOOL_COUNT, 5); }
    #[test]
    fn initialize() {
