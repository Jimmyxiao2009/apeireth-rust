...
fn growth_page_does_not_show_culled_stages() {
    ...
    assert!(!out.contains("Reproduction"), "Growth 页不应含 Reproduction");
...
fn growth_page_shows_reflection_ring_status() {
    ...
    let has_ring = out.contains('◔') || out.contains('◐') || out.contains('◑')
...
fn growth_page_renders_4_cards_not_8() {
    ...
    let stage_count = ["Init", "Bootstrap", "Serving", "Saturated"]
        ...
        .count();
    assert_eq!(stage_count, 4, "Growth 页应渲染 4 卡 (R26 砍 8): {stage_count}");
}
