...
fn six_anchors_hardcoded_exact_6_with_correct_ids() {
    ...
    let ids: Vec<&str> = organ::mind::SIX_ANCHORS.iter().map(|(id, _, _)| *id).collect();
...
fn three_growth_stages_no_old_no_death() {
    ...
    assert_eq!(organ::mind::FOUR_STAGES, &["Init", "Bootstrap", "Serving", "Saturated"], "4 阶段工程用语");
