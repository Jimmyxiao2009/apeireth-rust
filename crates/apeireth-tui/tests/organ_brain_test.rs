...
fn render_contains_3_state_fields() {
    ...
    assert!(out.contains("循环") || out.contains("llm_calls"), "brain render 应含 cycles (R22 ST-A1.1)");
...
fn render_marks_partial_honestly() {
    ...
    assert!(out.contains("[BRAIN]") || out.contains("脑"), "brain render 不再假装 partial, 实际业务语义: {out}");
}
