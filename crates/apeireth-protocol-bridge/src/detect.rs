...
mod tests {
    ...
    fn detect_anthropic_via_path() {
        ...
        assert_eq!(p, VcpProtocol::AnthropicMessages);
    }

    #[test]
    fn detect_openai_responses() {
        let p = detect_protocol(&hints_with_path("/v1/responses"));
        assert_eq!(p, VcpProtocol::OpenAIResponses);
    }
    ...
    fn detect_gemini() {
        ...
        assert_eq!(p, VcpProtocol::Gemini);
    }

    #[test]
    fn detect_unknown() {
        let p = detect_protocol(&hints_with_path("/random"));
        assert_eq!(p, VcpProtocol::Unknown);
    }
