...
impl DailyNoteTool {
    ...
    pub fn all() -> &'static [DailyNoteTool] {
        &[DailyNoteTool::NoteCreate, DailyNoteTool::NoteGet, DailyNoteTool::NoteSearch, DailyNoteTool::NoteExport]
    }
}
...
impl DailyNoteMcp {
    ...
    pub fn handle(&self, req: McpRequest) -> McpResponse {
        match req.method.as_str() {
            ...
            "tools/list" => {
                let tools: Vec<_> = DailyNoteTool::all().iter().map(|t| json!({
                    "name": t.name(),
                    "description": match t {
            ...
            "tools/call" => {
                ...
                let summary = match tool {
                    "note_create" => format!("create note title={:?}", args.get("title").and_then(|v| v.as_str()).unwrap_or("")),
                    "note_get" => format!("get note id={:?}", args.get("id").and_then(|v| v.as_str()).unwrap_or("")),
                    "note_search" => format!("search query={:?}", args.get("query").and_then(|v| v.as_str()).unwrap_or("")),
                    "note_export" => format!("export note id={:?} format={:?}", args.get("id").and_then(|v| v.as_str()).unwrap_or(""), args.get("format").and_then(|v| v.as_str()).unwrap_or("markdown")),
...
impl Default for DailyNoteMcp { fn default() -> Self { Self::new() } }

#[cfg(test)]
mod tests {
    use super::*;
    #[test]
    fn tool_count_is_4() { assert_eq!(DAILYNOTE_MCP_TOOL_COUNT, 4); }
    #[test]
    fn initialize_works() {
