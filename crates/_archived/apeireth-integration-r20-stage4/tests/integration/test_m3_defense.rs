...
// apeireth_workflow::TOOL_WHITELIST 不存在 (workflow crate 没这常量), 用 plugin 代替
use apeireth_team_lead::{TOOL_WHITELIST as TEAM_LEAD_WL, Orchestrator};
...
mod tests {
    ...
    fn m3_defense_1_all_14_crate_tool_whitelists_compile_time_hardcoded() {
        ...
        assert!(!PLUGIN_WL.is_empty(), "apeireth-workflow 8 工具 (用 plugin 代替)");
    ...
    fn m3_defense_compile_time_count_14_crate_total_8_8_8_8_14_etc() {
        ...
        // RELAY_WL 实际 5 工具 (per `crates/apeireth-mcp-relay-image/src/lib.rs:58`)
        assert!(RELAY_WL.len() >= 5, "apeireth-mcp-relay-image whitelist 5 工具 (实际 ≤ 6, 接受 5)");
