...
use crate::label::{validate_label_key, validate_labels, LABEL_MAX_COUNT};
...
impl ExporterKind {
    ...
    pub fn check_implemented(&self) -> crate::error::MetricsResult<()> {
        if self.is_implemented() {
            Ok(())
        } else {
            Err(crate::error::MetricsError::ExporterNotImplemented(
...
impl std::str::FromStr for ExporterKind {
    type Err = crate::error::MetricsError;
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.to_ascii_uppercase().as_str() {
            ...
            other => Err(crate::error::MetricsError::ExporterNotImplemented(
                other.to_string(),
            )),
        }
    }
...
impl MetricsConfig {
    ...
    pub fn default_config() -> Self {
        Self {
            ...
            subsystem: crate::DEFAULT_SUBSYSTEM.to_string(),
            global_labels: HashMap::new(),
            exporter: crate::DEFAULT_EXPORTER,
        }
    }
    ...
    pub fn validate(&self) -> crate::error::MetricsResult<()> {
        if self.namespace.is_empty() {
            return Err(crate::error::MetricsError::MetricNameInvalid(
                "namespace".to_string(),
            ));
        }
        ...
        if self.global_labels.len() > LABEL_MAX_COUNT {
            return Err(crate::error::MetricsError::TooManyLabels {
