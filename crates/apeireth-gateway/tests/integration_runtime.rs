...
use apeireth_bus::{ChannelSet, ChanneledBus};
...
impl AsyncWorker for FrameGatewayWorker {
    ...
    async fn execute(&self, task_id: TaskId, _params_json: String) -> Result<String, String> {
        ...
        let frame = transport
            .recv()
            .await
            .ok_or_else(|| "no inbound frame".to_string())?;
...
async fn gateway_admit_then_loopback_carries_frame() {
    ...
    let a = gw
        .admit_node(NodeKind::Cli, "client", "alice", "k1", 0)
        .unwrap();
    let loopback: Arc<InMemoryTransport> = gw.loopback();
    loopback.as_ref().start().await.unwrap();
    loopback
        .as_ref()
        .push_inbound(InFrame::new(a.node_id, "loopback", json!({"ping": 1})));
...
async fn runtime_registers_worker_and_dispatches() {
    ...
    let loopback: Arc<InMemoryTransport> = gw.loopback();
    loopback.as_ref().start().await.unwrap();
    let worker = Arc::new(FrameGatewayWorker {
        loopback: loopback.clone(),
    });
    rt.register_worker("frame-gateway-worker", worker);

    loopback.as_ref().push_inbound(InFrame::new(
        a.node_id,
        "loopback",
        json!({"hello": "world"}),
    ));
...
async fn runtime_single_cycle_after_gateway_dispatch() {
    ...
    loopback
        .as_ref()
        .push_inbound(InFrame::new(a.node_id, "loopback", json!({"step": 1})));
    let task_id = rt.dispatch_async_task("frame-gateway-worker", "{}").await;
    let _ = rt
        .task_store
        .wait_for_completion(task_id, Duration::from_secs(5))
...
async fn gateway_multiple_nodes_drive_runtime_in_parallel() {
    ...
    gw.register_key("k1", "alice", "primary", DmScope::All, 0);
    let a1 = gw
        .admit_node(NodeKind::Cli, "client-1", "alice", "k1", 0)
        .unwrap();
    let a2 = gw
        .admit_node(NodeKind::Http, "client-2", "alice", "k1", 0)
        .unwrap();
    ...
    loopback
        .as_ref()
        .push_inbound(InFrame::new(a1.node_id, "loopback", json!({"from": "cli"})));
    loopback.as_ref().push_inbound(InFrame::new(
        a2.node_id,
        "loopback",
        json!({"from": "http"}),
    ...
    let _ = rt
        .task_store
        .wait_for_completion(t1, Duration::from_secs(5))
        .await
        .unwrap();
    let _ = rt
        .task_store
        .wait_for_completion(t2, Duration::from_secs(5))
        .await
        .unwrap();
    ...
    assert!(
        outs.len() >= 1,
        "expected outbound frames, got {} (out={:?})",
        outs.len(),
        outs
    );
...
async fn gateway_loopback_publishes_to_runtime_bus() {
    ...
    gw.register_key("k1", "alice", "primary", DmScope::All, 0);
    let _ = gw
        .admit_node(NodeKind::Desktop, "desktop", "alice", "k1", 0)
        .unwrap();
    ...
    let result = _bus
        .publish_multi(
            ChannelSet::AI,
            "ai:test",
            apeireth_bus::BusMessage::new(RuntimeEvent::new(
                0,
                None,
                "smoke",
                "ai:test",
