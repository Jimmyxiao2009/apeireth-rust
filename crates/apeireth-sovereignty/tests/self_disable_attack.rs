...
use apeireth_sovereignty::self_disable::{SelfDisableCheck, SelfDisableGuard, SelfDisableSignal, SelfDisableTrigger};
...
fn attack_1_no_degrade_string_case_variation() {
    ...
    println!("attack_1: 5 variants → {} triggered (strict-lowercase whitelist)", triggered);
}
...
fn attack_3_no_patch_protected_rule_alias() {
    ...
    assert!(!r1.is_triggered(), "PascalCase alias not in whitelist (pass)");
...
fn attack_4_no_bypass_master_case_variation() {
    ...
    assert!(r1.is_triggered(), "lowercase master + bypass triggers (eq_ignore_ascii_case)");
    assert!(r2.is_triggered(), "uppercase master + bypass triggers");
    assert!(r3.is_triggered(), "exact Master token triggers");
    assert_eq!(g.record_count(), 3, "all 3 case variants trigger (case-insensitive)");
}
...
fn attack_10_mechanism_id_immutable() {
    let triggers = [
        SelfDisableTrigger::NoDegradeViolation { from: "high".into(), to: "low".into() },
        SelfDisableTrigger::NoPatchViolation { rule: "principle_keys_count".into() },
        SelfDisableTrigger::NoBypassViolation { token: "Master".into() },
        SelfDisableTrigger::NoReverseViolation { trigger_id: "sd-000001".into() },
        SelfDisableTrigger::NoHideViolation { window_id: "w1".into() },
    ];
...
fn attack_11_full_sweep_5_mechanisms_5_attempts_each() {
    ...
    for i in 0..5 {
        ...
        let _ = g.check_no_hide(&format!("audit-window-{i}"), &format!("sweep-{i}"), 1000 + i);
    }
    ...
    // trigger_id 全部唯一
    let ids: std::collections::HashSet<String> = g.records().iter().map(|r| r.trigger_id.clone()).collect();
    assert_eq!(ids.len(), 25, "trigger_id must be unique across 25 attempts");
}
