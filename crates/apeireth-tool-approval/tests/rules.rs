...
fn manager_short_circuits_on_first_match() {
    ...
    let decision = m.check(&call);
    assert!(matches!(decision, ApprovalDecision::Allow),
        "Trust should short-circuit before Blacklist, got {:?}", decision);
}
...
fn risk_rule_with_custom_categories() {
    ...
    let rule = RiskRule::with_categories(5 * 60 * 1000, vec!["db".to_string(), "shell".to_string()]);
    let call = parse_one("<<<[TOOL_REQUEST]>>>\ntool_name:<<<db_query>>>\narg:<<<x>>>\n<<<[END_TOOL_REQUEST]>>>");
    match rule.check(&call, &[]) {
...
fn risk_rule_is_high_risk_helper() {
    ...
    assert!(!rule.is_high_risk("hello_world"), "hello_world 不以高风险前缀开始");
    // 大小写不敏感
    assert!(rule.is_high_risk("SYSTEM_exec"));
}
...
fn frequency_rule_within_window_triggers_deny() {
    ...
    match d {
        ApprovalDecision::Deny { reason, silent } => {
            assert!(reason.contains("频率") || reason.contains("阈值"), "reason 应含频率说明: {reason}");
...
fn frequency_rule_different_tool_no_match() {
    ...
    use apeireth_tool_runtime::parser::ToolCallParser;
    let rule = FrequencyRule::with_limits(60_000, 3);
    let calls_a = ToolCallParser::parse("<<<[TOOL_REQUEST]>>>\ntool_name:<<<A>>>\n<<<[END_TOOL_REQUEST]>>>").expect("parse");
    let calls_b = ToolCallParser::parse("<<<[TOOL_REQUEST]>>>\ntool_name:<<<B>>>\n<<<[END_TOOL_REQUEST]>>>").expect("parse");
    let history = vec![
        CallRecord::new(&calls_a[0], ApprovalDecision::Allow, Some("trust".to_string())),
        CallRecord::new(&calls_a[0], ApprovalDecision::Allow, Some("trust".to_string())),
    ];
    // 当前是 B, 历史都是 A → 不应触发
    let d = rule.check(&calls_b[0], &history);
    assert!(matches!(d, ApprovalDecision::NoMatch), "不同工具应 NoMatch, 实际: {d:?}");
}
...
fn fuzzy_bridge_resolves_typo_to_registered_tool() {
    ...
    r.register("FileOperator".to_string(), Arc::new(MockSyncTool { name: "FileOperator".to_string() }));
...
fn manager_records_decision_to_history() {
    ...
    // history 应至少有 1 条
    assert!(mgr.history_len() >= 1, "history 应记录, got {}", mgr.history_len());
}
