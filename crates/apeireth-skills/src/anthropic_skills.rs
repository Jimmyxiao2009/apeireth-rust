...
impl SkillDocument {
    pub fn size_bytes(&self) -> usize {
        self.body_md.len()
            + self.references.iter().map(|p| p.metadata().map(|m| m.len() as usize).unwrap_or(0)).sum::<usize>()
            + self.scripts.iter().map(|p| p.metadata().map(|m| m.len() as usize).unwrap_or(0)).sum::<usize>()
            + self.assets.iter().map(|p| p.metadata().map(|m| m.len() as usize).unwrap_or(0)).sum::<usize>()
    }
}
...
impl AnthropicSkillLoader {
    ...
    pub fn scan_entries(&self) -> AnthropicSkillResult<Vec<SkillEntry>> {
        if !self.root.exists() {
            return Err(AnthropicSkillError::PathNotFound(self.root.display().to_string()));
        }
        if !self.root.is_dir() {
            return Err(AnthropicSkillError::NotDirectory(self.root.display().to_string()));
        }
        let mut entries = Vec::new();
        for dir in std::fs::read_dir(&self.root).map_err(|e| AnthropicSkillError::PathNotFound(e.to_string()))? {
            let dir = dir.map_err(|e| AnthropicSkillError::PathNotFound(e.to_string()))?;
            let path = dir.path();
            if !path.is_dir() { continue; }
            let skill_md = path.join("SKILL.md");
            if !skill_md.exists() { continue; }
            let raw = std::fs::read_to_string(&skill_md).map_err(|e| AnthropicSkillError::InvalidFrontmatter(e.to_string()))?;
            let manifest = parse_frontmatter(&raw)?;
            manifest.validate()?;
            entries.push(SkillEntry { manifest, skill_md_path: skill_md });
        }
        Ok(entries)
    }
    ...
    pub fn load_full(&self, name: &str) -> AnthropicSkillResult<SkillDocument> {
        ...
        if !skill_dir.is_dir() {
            return Err(AnthropicSkillError::PathNotFound(skill_dir.display().to_string()));
        }
        let skill_md = skill_dir.join("SKILL.md");
        if !skill_md.exists() {
            return Err(AnthropicSkillError::MissingSkillMd(skill_dir.display().to_string()));
        }
        let raw = std::fs::read_to_string(&skill_md).map_err(|e| AnthropicSkillError::InvalidFrontmatter(e.to_string()))?;
        ...
        let doc = SkillDocument { manifest, body_md: body, references, scripts, assets };
...
fn parse_frontmatter_and_body(raw: &str) -> AnthropicSkillResult<(SkillManifest, String)> {
    ...
    if !trimmed.starts_with("---") {
        return Err(AnthropicSkillError::InvalidFrontmatter("missing opening ---".into()));
    }
    ...
    let body = after_open[body_start..].trim_start_matches(|c: char| c == '\n' || c == ' ').to_string();
...
mod tests {
    ...
    fn manifest_validate_accepts_kebab() {
        let m = SkillManifest { name: "good-skill-1".into(), description: "ok".into(), ..Default::default() };
    ...
    fn loader_scan_and_load() {
        ...
        write_skill(&tmp, "skill-a", "name: skill-a\ndescription: First", "body of skill a");
        write_skill(&tmp, "skill-b", "name: skill-b\ndescription: Second", "body of skill b");
