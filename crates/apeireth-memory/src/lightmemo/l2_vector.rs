...
impl L2VectorStore {
    ...
    pub fn cosine_search(&self, query: &[f32], top_k: usize) -> Vec<(String, f32)> {
        let mut scored: Vec<(String, f32)> = self.entries.values()
...
mod tests {
    ...
    fn insert_and_search() {
        ...
        s.insert(VectorEntry { id: "a".into(), embedding: vec![1.0, 0.0] });
        s.insert(VectorEntry { id: "b".into(), embedding: vec![0.0, 1.0] });
        s.insert(VectorEntry { id: "c".into(), embedding: vec![1.0, 1.0] });
    ...
    fn remove_works() {
        let mut s = L2VectorStore::new();
        s.insert(VectorEntry { id: "x".into(), embedding: vec![1.0] });
        assert_eq!(s.count(), 1);
        let r = s.remove("x");
        assert!(r.is_some());
        assert_eq!(s.count(), 0);
