...
impl MemoryReliabilityStage {
    pub fn new(cache: std::sync::Arc<FingerprintCache>) -> Self {
        Self { cache }
    }
}
    ...
    fn process(
        ...
        if self.cache.check(&fp) {
            ...
            return Err(apeireth_pipeline_g5::PipelineError::Stage {
                ...
                source: "memory fingerprint dedup hit".into(),
            });
        }
        m.attempt += 1;
        Ok(m)
...
impl MemoryThrottleStage {
    pub fn new(throttle: std::sync::Arc<KeyThrottle>) -> Self {
        Self { throttle }
    }
}
    ...
    fn process(
        ...
        if !self.throttle.check_and_inc(&key) {
            return Err(apeireth_pipeline_g5::PipelineError::Stage {
                ...
                source: format!("key {} over rate limit", key).into(),
            });
        }
        Ok(m)
    }
...
impl MemoryPipelineBuilder {
    ...
    pub fn with_ttl(mut self, t: TtlPolicy) -> Self {
        self.ttl = t;
        self
    }
    ...
    pub fn build(self) -> Pipeline<MemoryPipeline, PipelineMessage, PipelineMessage> {
        let config = PipelineConfig::new(self.name, "MemoryPipeline");
        Pipeline::new(config)
            .with_stage(MemoryDispatchStage)
            .with_stage(MemoryNormalizeStage::new())
            .with_stage(MemoryPolicyStage::new(self.ttl))
            .with_stage(MemoryReliabilityStage::new(self.fingerprint_cache))
            .with_stage(MemoryThrottleStage::new(self.key_throttle))
    }
}
...
mod tests {
    ...
    fn dispatch_preserves_kind() {
        let s = MemoryDispatchStage;
        let m = PipelineMessage::new("note-insert", "x");
        let o = s.process(m).unwrap();
        assert_eq!(o.kind, "note-insert");
    }
    #[test]
    fn normalize_truncates_long_payload() {
        let s = MemoryNormalizeStage::new();
        let m = PipelineMessage::new("k", "a".repeat(2_000_000));
        let o = s.process(m).unwrap();
        assert!(o.payload.len() <= 1024 * 1024);
    }
    #[test]
    fn normalize_truncates_long_kind() {
        let s = MemoryNormalizeStage::new();
        let m = PipelineMessage::new("k".repeat(1000), "p");
        let o = s.process(m).unwrap();
        assert!(o.kind.len() <= 256);
    }
    #[test]
    fn policy_adds_ttl() {
    ...
    fn reliability_dedups() {
        ...
        let m = PipelineMessage::new("unique-mem-1", "p");
    ...
    fn throttle_rate_limit_per_key() {
        ...
        assert!(s.process(PipelineMessage::new("kind-a", "1")).is_ok());
        assert!(s.process(PipelineMessage::new("kind-a", "2")).is_ok());
        assert!(s.process(PipelineMessage::new("kind-a", "3")).is_err());
        assert!(s.process(PipelineMessage::new("kind-b", "1")).is_ok());
    }
    #[test]
    fn full_pipeline_runs() {
        let p = MemoryPipelineBuilder::new("mem").build();
        let r = p.run(PipelineMessage::new("episode-insert", "hello world"));
