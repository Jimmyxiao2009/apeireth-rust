...
impl EmotionEvent {
    ...
    pub const fn pad_delta(&self) -> Pad {
        match self {
            ...
            Self::UserCritique => Pad {
                p: -0.3,
                a: 0.3,
                d: -0.1,
            },
            Self::TaskSuccess => Pad {
                p: 0.3,
                a: 0.1,
                d: 0.2,
            },
            Self::TaskFailure => Pad {
                p: -0.4,
                a: 0.2,
                d: -0.2,
            },
...
impl EmotionEngine {
    ...
    pub fn response_style(&self) -> ResponseStyle {
        let snap = self
            ...
            .back()
            .cloned()
            .unwrap_or_else(EmotionSnapshot::neutral);
...
mod tests {
    ...
    fn t09_response_style_changes() {
        ...
        assert!(matches!(
            after,
            ResponseStyle::Cautious | ResponseStyle::Diplomatic
        ));
...
fn r234_03_auto_decay_returns_elapsed_secs() {
    ...
    assert!(
        p_after_apply > p_before,
        "praise 应提高 pleasure: {p_before} → {p_after_apply}"
    );
...
fn r234_04_auto_decay_at_explicit_time() {
    ...
    assert!(
        p_now < p_after,
        "10s 衰减后 pad.p 应下降: {p_after} → {p_now}"
    );
}
...
fn r234_05_auto_decay_at_zero_elapsed_noop() {
    ...
    let p_after = engine.current_pad().p;
    assert!(
        (p_before - p_after).abs() < 1e-5,
        "0 elapsed 时 pad.p 应不变: {p_before} vs {p_after}"
    );
}
...
fn r237_01_decay_snapshot_records_pad_before_and_after() {
    ...
    assert!(
        (snap.elapsed_secs - 3.0).abs() < 1e-3,
        "elapsed_secs should be ~3.0"
    );
    // Before != After (decay pulls toward baseline)
    assert!(snap.drift() >= 0.0);
    assert!(
        snap.pad_before.p > snap.pad_after.p,
...
fn r243_02_history_since_filters_by_timestamp_threshold() {
    ...
    assert!(
        recent.len() >= 2,
        "should include both events, got {}",
        recent.len()
    );
    for s in &recent {
...
fn r244_03_apply_batch_sum_returns_total_resonance() {
    ...
    let sum = engine.apply_batch_sum(&[EmotionEvent::UserPraise, EmotionEvent::UserCritique]);
