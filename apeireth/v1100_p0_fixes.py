...
def fix_snapshot(project_dir: Path) -> Dict[str, Any]:
    """删除可重建的超限 snapshot，并清理 V1100 制造的重复巨型归档。

    snapshot 是 V1074 的派生产物，不是源数据；保留 21GB 副本不构成瘦身。
    删除前写一个小型 manifest 记录文件大小和时间，随后由 ``--report`` 重建。
    ponytail: ceiling = JSON snapshot；需要长期历史时升级为 SQLite WAL。
    """
    artifacts = project_dir / "artifacts"
    snap = artifacts / "asi_snapshot.json"
    archive_dir = artifacts / "_archive_v1100"
    result: Dict[str, Any] = {
        "action": "fix_snapshot", "before_bytes": 0, "after_bytes": 0,
        "removed_archives": [],
    }

    # 旧实现把 21GB 派生文件移动到归档目录，未释放任何空间；清理这些副本。
    if archive_dir.exists():
        for archive in archive_dir.glob("asi_snapshot_archived_*.json"):
            if archive.is_file() and archive.stat().st_size > MAX_SNAPSHOT_BYTES:
                result["removed_archives"].append(str(archive.relative_to(project_dir)))
                archive.unlink()

    if not snap.exists():
        result["note"] = "snapshot 不存在；已清理超限归档（如有），等待 V1074 重建"
        return result
    ...
    manifest = archive_dir / "asi_snapshot_removed_manifest.json"
    manifest.write_text(json.dumps({
        "removed_path": str(snap.relative_to(project_dir)),
        "removed_bytes": result["before_bytes"],
        "removed_at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "reason": "recursive derived snapshot; regenerated by V1074 --report",
    }, indent=2), encoding="utf-8")
    snap.unlink()
    result["manifest"] = str(manifest.relative_to(project_dir))
    result["note"] = "超限派生 snapshot 已删除；V1074 --report 将重建小型快照"
    return result
...
# ---------------------------------------------------------------------------
def verify_v1074(project_dir: Path, timeout: int = 300) -> Dict[str, Any]:
    """V1100 V1074 真测: --report 必须在完整测量预算内出 Markdown。"""
...
def fix_all(project_dir: Path) -> Dict[str, Any]:
    ...
    v74 = verify_v1074(project_dir, timeout=300)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.verify:
        summary["verify_v1074"] = verify_v1074(project_dir, timeout=300)
