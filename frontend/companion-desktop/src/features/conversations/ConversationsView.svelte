...
<section class="view">
  <PageHeader eyebrow="工作区" title="对话管理" subtitle="全局与项目对话分开保存；归档不会删除本地记录。">
    <button class="quiet-button" onclick={() => showArchived = !showArchived}>
      <Archive size={14}/>{showArchived ? '查看当前' : '查看归档'}
    </button>
  ...
  <div class="conversation-list">
      ...
      <article class:active={conversation.id === activeId}>
        <button class="conversation-open" onclick={() => onOpen(conversation.id)}>
          <div>
            ...
            <span class="badge" class:blue={conversation.scope !== 'project'} class:amber={conversation.scope === 'project'}>
              {conversation.scope === 'project' ? '项目' : '全局'}
            </span>
        ...
        <div class="conversation-actions">
          <button
            title={conversation.archived ? '恢复' : '归档'}
            aria-label={conversation.archived ? '恢复' : '归档'}
    ...
    {:else}
      <div class="blank-state">
        ...
        <h3>没有{showArchived ? '归档' : '当前'}对话</h3>
        <p>新建一个对话，开始独立的上下文。</p>
      </div>
    {/each}
  </div>
