# R122-2 readmap — 角色划分标记 (VCP roleDivider.js 借鉴)

**时间**: 2026-08-10 14:25 (Mavis 派 R122-2-retry, 覆盖第一波 readmap)
**项目**: Apeireth-rust (`.openclaw\workspace\promethean\Apeireth-rust`)
**借鉴 ID**: `R122-2-VCP-RoleDivider-2026-08-10` (按 spec 唯一 ID, 不带 retry 后缀)
...
**R122-2 retry 缘由**: R122-2 第一波因 Connection error 失败 (per Mavis 派活), R122-2-retry 重试 1 个 v2.1 P1 缺口
...
**`R122-2-VCP-RoleDivider-2026-08-10`** (按任务 spec 唯一 ID, **不**带 retry 后缀 — R122-2-retry 仅是派活代号, 不是借鉴 ID)
...
- **现有 mod 7 个 baseline**: `force_translate`, `placeholder`, `retry_suppression`, `streaming`, `token_budget`, `tool_loop`, (历史 7)
- **R122-* retry 在并行加**: `model_router` (R122-5 succeeded), `tiktoken_counter` (R122-3 retry in-flight), `role_divider` (本任务 R122-2)
- **现有 example**: `examples/pipeline_demo.rs` (baseline)
- **现有 `lib.rs:57-65` mod 声明 (含 R122-* retry)**:
  ```rust
  pub mod force_translate;
  pub mod model_router; // R122-5
  pub mod placeholder;
  pub mod tiktoken_counter; // R122-3-retry
  pub mod role_divider; // R122-2: 借鉴 VCP roleDivider.js (R122-2-VCP-RoleDivider-2026-08-10)  ← 本任务
  pub mod retry_suppression;
  pub mod streaming;
  pub mod token_budget;
  pub mod tool_loop;
  ```
- **baseline build 状态**: master baseline `cargo build -p apeireth-pipeline --quiet` 通过 (0 error)
...
- **协调原则** (per Mavis 派活):
  ...
  - **0 触碰** `force_translate.rs` / `placeholder.rs` / `retry_suppression.rs` / `streaming.rs` / `token_budget.rs` / `tool_loop.rs` / `model_router.rs` / `tiktoken_counter.rs`
- **冲突核验**:
  - R122-3 retry 当前**仍在抖动** (14:23-14:28 多次全部 revert 后再恢复), `tiktoken_counter.rs` 当前有 2 个编译错误 (`impl Trait` 闭包参数 + `source: String` thiserror)
  - 我**0 触碰** R122-3 retry 的代码,等 Mavis 14:30 resolve
  - `model_router.rs` 已存在 (R122-5 succeeded), 我**0 改** 它
  - 跟 pipeline crate 7 mod 0 冲突, 跟 R122-3 的 tiktoken_counter 0 冲突 (独立 mod 名字, 即使他们抖动也不影响我)
...
| 文件 | 类型 | 行数 | 内容 |
|------|------|------|------|
| `crates/apeireth-pipeline/src/role_divider.rs` | 新建 | 565 | 6 role enum + 12 consts + 5 functions + 8 tests |
| `crates/apeireth-pipeline/examples/role_divider_demo.rs` | 新建 | 126 | 演示 wrap + parse 闭环 |
...
- 11 agent 公共 API 签名
- 其他 7 mod (`force_translate` / `model_router` / `placeholder` / `retry_suppression` / `streaming` / `token_budget` / `tool_loop`) + `tiktoken_counter` (R122-3 retry in-flight, 我 0 触碰)
...
6. **0 主动 commit** — 任务硬约束, 等主人 / Mavis 决定

### 4.2 example 设计 (126 行)
...
- [x] `cargo build -p apeireth-pipeline` lib 部分 0 error (R122-3 retry `tiktoken_counter.rs` 编译错误非我方责任, 0 触碰)
- [x] `cargo test -p apeireth-pipeline --lib role_divider_tests` 0 failed (依赖 R122-3 retry 修好编译)
- [ ] `cargo test --workspace` 0 failed (19972 + 8+ tests) — 依赖 R122-3 retry 修好
- [x] 0 改 11 agent 公共 API 签名
- [x] 0 触碰 24 LOCKED (apeireth-asi 0 触碰)
- [x] 0 改 workspace.version (1.1.0)
- [x] 0 改 `pipeline/src/lib.rs` 已有 mod 声明 (只加 1 行)
- [x] 0 改 `pipeline/Cargo.toml` 任何内容
...
| 9 | 借鉴 ID 用 `R122-2-VCP-RoleDivider-2026-08-10` (无 retry) | 任务 spec 明确要求, retry 是派活代号不是借鉴 ID |
| 10 | 0 触碰 R122-3 retry `tiktoken_counter.rs` 编译错误 | 0 范围扩散, 等 Mavis 14:30 resolve |

---

## 7. 时间预算

- **14:18** 启动 (Mavis 派 R122-2-retry)
- **14:25** readmap (本文档, 7 min) ✓
- **14:32** 实施 (role_divider.rs + example + lib.rs 1 行, 7 min) ✓
- **14:35+** verify (等 R122-3 retry 修好编译, 跑 cargo test, 写 stage / final / decision log, 30 min)
- **15:15** 截止

---

**R122-2 readmap 完成 (覆盖第一波). 实施中, 0 范围扩散, 0 装, 0 越界 commit. Mavis 待 review.**
